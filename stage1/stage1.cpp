...
void ExecStmts()
{
    //cout << "XX:" << "ExecStmts: " << token << endl;
    if(token == ";")
    {
        NextToken();
    }
	//token must be a non key ID, "read", or "write" in order to be an exec statement, it can also be end if the program has looped
	if(!IsNonKey(token) && token != "read" && token != "write" && token != "end")
	{
		printError("non-keyword identifier,\"read\", or \"write\" expected");
	}
...
void Factors()
{
    ...
    else if(!RelOp() && !AddLevelOp() && token != ")" && token != ";")
    {
        //cout << "XX:" << "operator, \")\" or semicolon expected - FACTORS" << endl;
        printError("operator, \")\" or semicolon expected");
    }
}
...
allocation inAlloc, int inUnits, string forcedIntName)
{
	...
	{	
	    ...
	    else if(IsKeyWord(nameEntries[i]))
	    {
...
bool IsKeyWord(string str)
{
    return (str == "program" || str == "begin" || str == "end" || str == "var" || str == "const" || str == "integer" || str == "boolean" || str == "true" || str == "false" || str == "not" || str == "mod" || str == "div" || str == "and" || str == "or" || str == "read" || str == "write");
}
...
void pushOperand(string op)
{
	...
	if(find(symbolTable.begin(),symbolTable.end(),op) == symbolTable.end()) //has no symbol entry
	{
...
void EmitModCode(string operand1, string operand2)
{
	...
	{
	    //emit code to load operand2 into the A register;
	    objectFile << rammTab << "LDA " << setw(4) << left <<intOp2 << right << endl;
	}
    //divide by op1
...
void EmitNotEqualsCode(string operand1,string operand2) //test whether operand2 equals operand1
{
	...
	{
		 //emit code to load operand2 into the A register;
		objectFile << rammTab << "LDA " << left <<intOp2 << right << endl;
	}
    //emit code to perform a register-memory subtraction with A Register holding the result;
	objectFile << rammTab << "ISB " << left <<intOp1 << right << rTbFv << "  " << operand2 << " <> " << operand1 << endl;
...
void EmitAssignCode(string operand1,string operand2) //assign the value of operand1 to operand2
{
	...
	if (areg != intOp1)
	{
		...
		objectFile << rammTab << "LDA " <<  intOp1 << rTbFv << endl;
	}
