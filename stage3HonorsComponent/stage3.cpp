...
// Major honors debug. changed all instances where we check 
// if an operand is a temporary, so they don't look for 
// just the first letter being 'T', to avoid seeing TRUE
// as a temporary
using namespace std;
...
//honors additions
//For Loop functions and variables
void ForStmt();
void ToStmt();
void DowntoStmt();
void EmitForCode();
void EmitToCode(string operand1, string operand2);
void EmitPostToCode(string operand);
void EmitDowntoCode(string operand1, string operand2);
void EmitPostDowntoCode(string operand);
void EmitPostForCode(string operand1, string operand2);
bool ONE = false;
string loopControl;
//Case statement functions and variables
void CaseStmt();
void EmitCaseCode(string operand1, string operand2);
void EmitPostCaseCode(string operand);
void EmitPostConditionCode(string notEqualLabel);
void EmitEndCaseCode();
string postCaseLabel = "";
vector<string> caseConditions;
string listLabel = "";
...
{	
   ...
   {
      listingFile << "STAGE3:  " << "Anna Porter - Honors Component\t" << ctime(&now);
...
void PrintSymbolTable()
{
	...
	//Print header
	time_t now = time (NULL);
	out << "STAGE3:  " << "Anna Porter - Honors Component\t" << ctime(&now) << "\nSymbol Table\n\n";
	objectFile << out.str();
	out.str("");
...
{
   //cout << "XX: BEGIN END STMT " << token << endl;
   if (token != "begin")
   {
      printError("keyword \"begin\" expected"); //process err 
   }
   ...
   --beginDepth;
   NextToken();
   //cout << "Inside BeginEndStmt NextToken = " << token << endl;
   // *Honors debug for stage 0* 
   if (token != "." && beginDepth == 0)
   {
      printError("period expected"); //process err
   }
   if (beginDepth != 0 && token != ";")
   {
...
void ConstStmts() //token should be NON_KEY_ID
{
   ...
   if (y == "not")
   {
      ...
      // *honors debug for 131* whichValue is going to return 1,
      // when token is true;
      if(token == "true" || WhichValue(token) == "1") // || WhichValue(token) == "true")
      {
         //cout << "token" << endl;
         y = "false";
      }
...
void ExecStmts()
{
   //cout << "XX: EXEC STMTS " << token << endl;
   ExecStmt();
   //If token is not end, then it must be another exec stmt, so continue
   
   //*honors debug for stage0* Added a bool called errFound to switch to true when we find an error.
   // prevents infinite calls to execstmts;
	if(token != "end" && token != "until" && token != ")" && !errFound)
	{
		ExecStmts();
	}
}
...
void ExecStmt()
{
   ...
   //honors stage 3 added case and for and to and downto and do to this list
	if(!IsNonKey(token) && token != "read" && token != "write" && token != "end" && token != "if" && token != "while" && token != "repeat" && token != ";" && token != "begin"  && token != "until" && token != "case" && token != "for" && token != "to" && token != "downto" && token != "do")
	{
		printError("non-keyword identifier, \"read\", \"write\", \"if\", \"while\", \"repeat\", \"case\", \"for\", \";\", or \"begin\" expected: top of exec.");
	}
   ...
   // honors stage3 added for and case;
   else if(token == "for")
   {
      ForStmt();
   }
   ...
   {
      printError("non-keyword identifier, \"read\", \"write\", \"if\", \"while\", \"repeat\", \";\", or \"begin\" expected: bottom of exec");
   }
}
...
void AssignStmt()
{
   ...
   //honors stage3 added to and downto
   if(token != ";" && token != "to" && token != "downto")
   {
...
void Expresses()
{
   ...
   //Honors added of to this list and to and downto
   else if(token != ")" && token != "," && token != ";" && token != "do" && token != "then" && token != "of" && token != "to" && token != "downto")
   {
...
void Terms()
{
   ...
   //honors stage 3 added of to this list and to and downto
   else if(!RelOp() && token != ")" && token != ";" && token != "do" && token != "then" && token != "of" && token != "to" && token != "downto")
   {
...
void Factors()
{
   ...
   //honors added token != of and != to and != downto
   else if(!RelOp() && !AddLevelOp() && token != ")" && token != ";" && token != "do" && token != "then" && token != "of" && token != "to" && token != "downto")
   {
      // honors debug for 114
      // This should be specific enough that it never gets triggered under any other circumstance
      if(token == "5")
      {
         printError("expected '(', integer, or non-keyword id; found +");
      } 
      else 
      {
         //cout << token << " +++++++++++++++++++++++++++++++" << endl;
...
allocation inAlloc, int inUnits, string forcedIntName)
{
   ...
   //cout << "XX: INSERT " << externalName << endl;
	...
	{	
      ...
      {
         // *honors debug for stage 0* updated the error to include "-- max 256 entries"
         printError("symbol table overflow -- max 256 entries");
      }
...
{
   //cout << "WhichType" << endl;
   //honors debug stage1 141
   if(name.size() > 15)
   {
      name = name.substr(0, 15);
      //cout << "WHICHVALUE = " << name << "-----------------" << endl;
   }
	storeType type;
...
//Honors stage. added case, of, for, to and downto
bool IsKeyWord(string str)
{
...
void pushOperand(string op)
{
   ...
   // Honors debug for 141, to handle those long operators.
   if(op.size() > 15)
   {
      op = op.substr(0, 15);
   }
   if(op!= "" && op[0] != 'L')
   {
      if(op != "" && op[0] != 'L' && find(symbolTable.begin(),symbolTable.end(),op) == symbolTable.end()) //has no symbol entry
      {
         ...
         //honors debug for 116. adding this in from stage1 bc it did 116 right
         else if (op == "yes")
         {
            Insert(op, BOOLEAN,CONSTANT,"1",YES,1);
         }
...
void Code(string op, string operand1, string operand2)
{
   ...
   //honors stage3 added case and postCase to code.
   else if (op == "case")
   {
      EmitCaseCode(operand1, operand2);
   }
   else if (op == "postCase")
   {
      EmitPostCaseCode(operand1);
   }
   else if (op == "end_case")
   {
      EmitEndCaseCode();
   ...
   {
      EmitDowntoCode(operand1, operand2);
   }
   else if (op == "post_for")
   {
      EmitPostForCode(operand1, operand2);
   }
   else if (op == "post_downto")
   {
      EmitPostDowntoCode(operand1);
   }
   else if (op == "post_to")
   {
      EmitPostToCode(operand1);
   }
   else if (op == "post_condition")
   {
      EmitPostConditionCode(operand1);
   }
   else
   {
      ...
      if(entr.alloc == YES)
      {
         ...
         if(entr.mode == CONSTANT)
         {
            ...
            else
            {
               ...
               // *honors debug for stage0* Getting a coredump with std::out_of_range from this at down here, so we check to see if we found an error before we call .at(0)
               if(entr.value.size() > 0 && entr.value.at(0) == '-')
               {
...
void EmitNegationCode(string operand1)
{
   ...
   // Honors debug stage1 133, even if intAreg == intOp1, we still need to store it, because we're about to load zero and subract that value.
   ...
   //A register holds a non-temp not operand1 nor operand2 then deassign it
	if (intAreg[0] != 'T'  && isdigit(intAreg[1]) && intAreg != intOp1)
	{
      areg = "";
      intAreg = "";
	}
   if(operand1.at(0) == 'T' && isdigit(operand1[1]))
      FreeTemp();
   areg = GetTemp();
...
void EmitWriteCode(string operand1)
{
	...
	for(unsigned int i=0; i<nameEntries.size(); ++i)
	{
      ...
      //honors debug stage1 I don't think write should update the areg. but idk...
      //stage1 did LDA I0 for 136 and got it right so i changed this
      //areg = nameEntries[i];
	}
}
...
void EmitAdditionCode(string operand1,string operand2) //add operand1 to operand2
{
   ...
   //A register holds a non-temp not operand1 nor operand2 then deassign it
	...
	if(intAreg[0] != 'T' && intAreg != intOp1 && intAreg != intOp2)
	{
	   ...
	   objectFile << rammTab << "LDA " << setw(4) << left << left <<intOp2 << right <<  rTbFv << endl;
		intAreg = intOp2;
	}
   ...
   if(operand1[0] == 'T' && isdigit(operand1[1]))
   {
      FreeTemp();
   }
   if(operand2[0] == 'T' && isdigit(operand2[1]))
   {
      FreeTemp();
   }	
...
void EmitSubtractionCode(string operand1, string operand2)
{
   //cout << "EMIT SUBTRACTION CODE" << endl;
...
{
   //cout << "XX: EMIT MULT CODE: " << operand1 << " " << operand2 << endl; 
   string intOp1, intOp2, intAreg; //internal names for operand 1 and 2 and areg
   intAreg = getIntName(areg);
   intOp1 = getIntName(operand1);
   intOp2 = getIntName(operand2);
...
void EmitOrCode(string operand1, string operand2)
{
   ...
   if(areg != "" && areg.at(0) == 'T' && isdigit(intAreg[1]) && (areg != operand1 && areg != operand2))
   {
	   //emit code to store that temp into memory
		objectFile << rammTab << "STA " << setw(4) << left << intAreg << right << rTbFv << "deassign AReg" << endl;
		//change the allocate entry for the temp in the symbol table to yes
   ...
   if(operand1.at(0) == 'T' && isdigit(operand1[1]))
      FreeTemp();
   if(operand2.at(0) == 'T' && isdigit(operand2[1]))
      FreeTemp();
   areg = GetTemp();
   (*find(symbolTable.begin(),symbolTable.end(),areg)).dataType = BOOLEAN;
   pushOperand(areg);
...
void EmitNotCode(string operand)
{
   //cout << "XX: EmitNotCode for Operand: " << operand << endl;
   ...
   if(operand.at(0) == 'T' && isdigit(operand[1]))
   {
      FreeTemp();
   }
...
void EmitEqualsCode(string operand1,string operand2) //test whether operand2 equals operand1
{
   ...
   //if A Register holds a temp not operand1 nor operand2 then 
   if(intAreg[0] == 'T' && isdigit(intAreg[1]) && (intAreg != intOp1 && intAreg != intOp2))
   {
      //emit code to store that temp into memory
   	objectFile << rammTab << "STA " << setw(4) << left << intAreg << right << rTbFv << "deassign AReg" <<endl;
      //change the allocate entry for it in the symbol table to yes 
   ...
   // *honors debug stage1 removed << rmtbFv after the +1
...
void EmitAssignCode(string operand1,string operand2) //assign the value of operand1 to operand2
{
   ...
   intOp2 = getIntName(operand2);
	...
	if(intOp1[0] == 'T' && isdigit(intOp1[1]))
	{
	   FreeTemp();
	   areg = "";
	}
   areg = operand2;
   //cout << "emit assign areg is now " << areg << endl;
	//free temporary
...
void EmitStartCode()
{
   ...
   if(ent.dataType != PROG_NAME)
   {
	...
	objectFile << "STRT  NOP" << rTbFv << rTbFv << ent.externalName << " - Anna Porter" << endl;
}
...
void EmitThenCode(string operand) //emit code that follows 'then' and statement predicate
{
	...
	if(WhichType(operand) != BOOLEAN)
   {
	...
	if(operand[0] == 'T' && isdigit(operand[1]))
	{
...
//HONORS STAGE3 PRODUCTIONS.
void CaseStmt() {
   //cout << "Case Statement: " << token << endl;
   if(token != "case") {
      printError("Keyword \"case\" expected");
   }
   // A case statement has "end" after the entire case statement is over
   // so we increment the begin depth after seeing the word case.
   beginDepth++;
   NextToken();
   Express();
   if(token != "of") {
      printError("Keyword \"of\" expected");
   }
   string y = popOperand();
   NextToken();
   while(token != "end" && !errFound)
   {
      if(WhichType(y) != WhichType(token))
      {
         printError("The type of label must match the type of the case condition");
      }
      if(caseConditions.size() > 0)
      {
         for(int i = 0; i < caseConditions.size(); i++)
         {
            if(token == caseConditions[i])
            {
               printError("Can not have duplicate case labels");
            }
         }
      }
      //cout << "push_back " << token << endl;
      caseConditions.push_back(token);
      pushOperand(token);
      listLabel = "";
      Code("case", y, popOperand());
      NextToken();
      int listNum = 1;
      while (token == ",")
      {
         ...
         if(caseConditions.size() > 0)
         {
            for(int i = 0; i < caseConditions.size(); i++)
            {
               if(token == caseConditions[i])
               {
                  //cout << token << " and " << caseConditions[i] << endl;
                  printError("Can not have duplicate case labels");
               }
            }
         }
      ...
      Code("post_condition", popOperand());
      if(token != ":")
      {
         printError("\":\" expected");
      }
      NextToken();
      //cout << "inside case stmts" << token << endl;
      ExecStmt();
      Code("postCase", popOperand());
   }
   if(token != "end")
   {
   ...
   NextToken();
   NextToken();
   //cout << "clear caseconditions" << endl;
   caseConditions.clear();
}

// operand 1 is the name of our case condition
// operand 2 is an integer or boolean
// Check for equality and jump to the equality
// Label if it is equal.
void EmitCaseCode(string operand1, string operand2)
{
   string intOp2 = getIntName(operand2);
   string intAreg = getIntName(areg);
   ...
   if(postCaseLabel == "")
   {
      ...
      newLabel << "L" << loopCount;
      postCaseLabel = newLabel.str();
      loopCount++;
   }
   if(listLabel == "")
   {
      ostringstream equalityLabel;
      equalityLabel << "L" << loopCount;
      listLabel = equalityLabel.str();
      loopCount++;
   }
   if(areg[0] == 'T' && isdigit(areg[1]))
   {
      objectFile << rammTab << "STA " << setw(4) << left << areg << rTbFv << endl;
            //change the allocate entry for it in the symbol table to yes 
   	(*find(symbolTable.begin(),symbolTable.end(),areg)).alloc = YES;
   }
   if(intAreg != intOp1)
   ...
   objectFile << rammTab << "ISB " << setw(4) << left << intOp2 << right << rTbFv << operand1 << " = " << operand2<< endl;
   areg = "";
   objectFile << rammTab << "AZJ " << setw(4) << left << listLabel << right << rTbFv << endl;

   //pushOperand(notEqualLabel.str());
   pushOperand(listLabel);
   //pushOperand(notEqualLabel.str());
}
void EmitPostConditionCode(string equalityLabel)
{
   ostringstream notEqualLabel;
   notEqualLabel << "L" << loopCount;
   loopCount++;
   objectFile << rammTab << "UNJ " << setw(4) << left << notEqualLabel.str() << right << rTbFv <<  endl;
   objectFile << setw(6) << left << equalityLabel << "NOP" << rTbFv << rTbFv << endl;
   pushOperand(notEqualLabel.str());
}

void EmitPostCaseCode(string operand)
{
   objectFile << rammTab << "UNJ " << setw(4) << left << postCaseLabel << rTbFv << endl;
   objectFile << setw(6) << left << operand << "NOP"  << rTbFv << rTbFv << endl;
}
void EmitEndCaseCode()
{
   objectFile << setw(6) << left <<  postCaseLabel << "NOP" << rTbFv << rTbFv << endl;
   postCaseLabel = "";
}
...
void ForStmt() {
   if(token != "for") {
      printError("Keyword \"for\" expected");
   }
   //cout << "inside ForStmt: " << "calling NextToken" << endl;
   NextToken();
   //Need the variable after for on our stack
   loopControl = token;
   ExecStmt();
   Code("for");
   pushOperand(loopControl);
   //cout << "Inside ForStmt: " << token << endl;
   if(token != "to" && token != "downto") {
      printError("Keyword \"to\" or \"downto\" expected");
   }
   //pushOperand(areg);
}
...
void ToStmt()
{
   if(token != "to")
   {
      printError("Keyword \"to\" expected");
   }
   NextToken();
   Express();
   Code("to", popOperand(), popOperand());
   if(token != "do")
   {
      printError("Keyword \"do\" expected");
   }
   NextToken();
   ExecStmt();
   Code("post_to", loopControl);
   Code("post_for", popOperand(), popOperand());
}
...
void DowntoStmt() 
{
   if(token != "downto")
   {
      printError("Keyword \"downto\" expected");
   }
   NextToken();
   Express();
   Code("downto", popOperand(), popOperand());
   if(token != "do")
   {
      printError("Keyword \"do\" expected");
   }
   NextToken();
   ExecStmt();
   Code("post_downto", loopControl);
   Code("post_for", popOperand(), popOperand());
}

void EmitForCode()
{
   ostringstream topOfFor;
   topOfFor << "L" << loopCount;
   loopCount++;
   objectFile << setw(6) << left << topOfFor.str() << "NOP" << rTbFv << rTbFv << "top of for loop" << endl;
   pushOperand(topOfFor.str());
}
...
void EmitToCode(string operand1, string operand2)
{
   ...
   string intOp2 = getIntName(operand2);
   string intAreg = getIntName(areg);
   // If there is an expression after "to", we don't need to load anything. The temporary after the expression is evaluated and already in the areg, so we don't need to load anything.
   if (WhichType(operand1) != INTEGER || WhichType(operand2) != INTEGER) 
	{
		printError("for loop conditions must be integer operands");
	}
   if(operand1[0] == 'T' && isdigit(operand1[1]))
   {
      areg = "";
   ...
   else // we are using a temporary, but we need to free the areg and free a temp
   {
      objectFile << rammTab << "LDA " << setw(4) << left << intOp1 << right << rTbFv << endl;
   }
   objectFile << rammTab << "ISB " << setw(4) << left << intOp2 << right << rTbFv << endl;
   ostringstream endOfFor;
   endOfFor << "L" << loopCount;
   loopCount++;
   objectFile << rammTab << "AMJ " << setw(4) << left << endOfFor.str() << rTbFv << operand2 << " to " << operand1 << endl;
   pushOperand(endOfFor.str());
   areg = "";
}
...
void EmitDowntoCode(string operand1, string operand2)
{
   ...
   if(areg[0] == 'T' && isdigit(areg[1]))
   {
      objectFile << rammTab << "STA " << areg << endl;
      (*find(symbolTable.begin(),symbolTable.end(),areg)).alloc = YES;
   }
   objectFile << rammTab << "LDA " << setw(4) << left << intOp2 << right << rTbFv << endl;
   objectFile << rammTab << "ISB " << setw(4) << left << intOp1 << right << rTbFv << endl;
   ostringstream endOfFor;
   endOfFor << "L" << loopCount;
   loopCount++;
   objectFile << rammTab << "AMJ " << setw(4) << left << endOfFor.str() << rTbFv << operand2 << " downto " << operand1 << endl;
   pushOperand(endOfFor.str());
   areg = "";
}
...
void EmitPostToCode(string operand)
{
   if(!ONE)
   {
      Insert("ONE", INTEGER, CONSTANT, "1", YES, 1, "ONE");
      ONE = true;
   }
   string intOp = getIntName(operand);
   string intAreg = getIntName(areg);
   ...
   objectFile << rammTab << "IAD ONE" << rTbFv << " increment loop condition" << endl;   
   objectFile << rammTab << "STA " << intOp << "  " << rTbFv << endl;
   areg = ""; // aregister is p much garbage
   }
...
void EmitPostDowntoCode(string operand)
{
   ...
   objectFile << rammTab << "ISB ONE" << rTbFv << " decrement loop condition" <<  endl;
   objectFile << rammTab << "STA " << intOp << rTbFv << endl;
   areg = ""; // aregister is p much garbage
}
...
void EmitPostForCode(string operand1, string operand2)
{
   objectFile << rammTab << "UNJ " << setw(4) << left << operand2 << right << rTbFv << endl;
   objectFile << setw(6) << left << operand1 << "NOP" << rTbFv << rTbFv << "end of for loop" << endl;
}
